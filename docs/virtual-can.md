...
This implementation temporarily develops against the reviewed m1-can runtime
model revision. It must not be published while that contributor revision is in
`Cargo.lock`. Publication requires the upstream m1-can changes and v0.2.3 (or a
later compatible release), followed by replacing the temporary revision and
rerunning all gates.
